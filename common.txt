...
function FnAjaxComplete(jqXHR,textStatus){	
	...
	$("#GBL_loading").hide();
	if(VarResponse.errorMessage != null && VarResponse.errorMessage != undefined){
		if(VarResponse.status == "UNAUTHORIZED"){
			$("#tokenexpiremodal").modal({show: 'true',backdrop:'static',keyboard: false}).find("#tokenexpiremodal-content").html('Your session has expired. Please <a href="/FMS/login">log in</a> again');
		}
	}
}

function FnErrcallback(jqXHR,textStatus) {
	var VarResponse = $.parseJSON(jqXHR.responseText);
	$("#GBL_loading").hide();
	if(VarResponse.status == "UNAUTHORIZED"){
		$("#tokenexpiremodal").modal({show: 'true',backdrop:'static',keyboard: false}).find("#tokenexpiremodal-content").html('Your session has expired. Please <a href="/FMS/login">log in</a> again');
	} else {
		FnShowNotificationMessage(VarResponse.errorMessage);
	}
}
...
function FnHandlePageActionNotification(ObjPageActionResponse){
	if(ObjPageActionResponse != ''){
		ObjPageActionResponse = $.parseJSON(ObjPageActionResponse);
		notificationMsg.show({
			message : ObjPageActionResponse.message
		}, ObjPageActionResponse.status);
	}
}
...
function logMsg(msg,color){	
	console.log('%c'+ msg, 'background:'+ color+'; color: white; line-height: 20px; display: block;padding:0 12px');
	
}

function FnAllowAlphaNumericWithSpace(e){
